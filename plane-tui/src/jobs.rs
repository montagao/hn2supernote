...
pub fn spawn(job: &Job, dir: &Path, claude_permission_mode: &str) -> Result<()> {
    ...
    spawn_raw(
        ...
        &job.tmux_session,
        &job.worktree,
        &format!("bash {}", shell_quote(&dir.join("run.sh").display().to_string())),
        &dir.join("log.txt"),
    )
}
...
pub fn pump(handle: &mut JobHandle) -> bool {
    ...
    let next = if let Some(code) = read_exit(&handle.dir) {
        if code == 0 {
            if read_result(&handle.dir).trim_start().starts_with("QUESTION:") {
...
mod tests {
    ...
    fn worktree_create_diff_and_discard() {
        ...
        git(&["add", "fix.txt"], &worktree);
        {
            ...
            commit
                .arg("-C")
                .arg(&worktree)
                .args(["-c", "user.email=t@t", "-c", "user.name=t", "commit", "-qm", "fix"]);
            run_quiet(commit).unwrap();
        }
        let job = Job {
            id: "t".into(),
        ...
        branches.arg("-C").arg(&repo).args(["branch", "--list", "tm-1-test"]);
        assert!(
    ...
    fn tmux_spawn_monitor_and_exit() {
        ...
        assert_eq!(handle.job.status, JobStatus::Review, "tail: {:?}", handle.tail);
        assert!(
